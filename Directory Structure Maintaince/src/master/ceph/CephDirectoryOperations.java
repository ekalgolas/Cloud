...
public class CephDirectoryOperations implements ICommandOperations {
	...
	private MetaDataServerInfo getRequiredMdsInfo(final Inode inode, final boolean isWrite) {
		...
		for (final MetaDataServerInfo info : inode.getDataServerInfo()) {
			if (Globals.ALIVE_STATUS.equalsIgnoreCase(info.getStatus())
					&& (isWrite && Globals.PRIMARY_MDS.equals(info.getServerType()) || !isWrite)) {
				serverInfo = info;
				return serverInfo;
			}
		}
	 ...
	 */
	private Message remoteExecCommand(final CommandsSupported command, final Inode inode, final String filePath,
	 ...
	 */
	private void create(final Directory root, final String currentPath, final String name, final boolean isFile,
	...
	@Override
	public void mkdir(final Directory root, final String path, final String... arguments)
			throws InvalidPropertiesFormatException {
