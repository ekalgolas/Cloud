 ...
 */
package dht.hdfs.server.common;
...
public final class HdfsServerConstants {
	...
	static public enum StartupOption {
		FORMAT("-format"), CLUSTERID("-clusterid"), GENCLUSTERID("-genclusterid"), REGULAR("-regular"), BACKUP(
				"-backup"), CHECKPOINT("-checkpoint"), UPGRADE("-upgrade"), ROLLBACK("-rollback"), FINALIZE(
						"-finalize"), IMPORT("-importCheckpoint"), BOOTSTRAPSTANDBY(
								"-bootstrapStandby"), INITIALIZESHAREDEDITS("-initializeSharedEdits"), RECOVER(
										"-recover"), FORCE("-force"), NONINTERACTIVE("-nonInteractive");
	...
	static public enum ReplicaState {
		...
		FINALIZED(0), /** Replica is being written to. */
		RBW(1), /** Replica is waiting to be recovered. */
		RWR(2), /** Replica is under recovery. */
		RUR(3), /**
				 * Temporary replica: created for replication and relocation
				 * only.
				 */
		TEMPORARY(4);
	...
	static public enum BlockUCState {
		...
		UNDER_CONSTRUCTION, /**
							 ...
							 * When a file lease expires its last block may not
							 * be {@link #COMPLETE} and needs to go through a
							 * recovery procedure, which synchronizes the
							 * existing replicas contents.
							 */
		UNDER_RECOVERY, /**
						 * The block is committed.<br>
						 * The client reported that all bytes are written to
						 * data-nodes with the given generation stamp and block
						 * length, but no {@link ReplicaState#FINALIZED}
						 * replicas has yet been reported by data-nodes
						 * themselves.
						 */
		COMMITTED;
	}
