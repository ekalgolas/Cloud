...
//            cin.close();
            ...
            for (int j = 1; j < replicaLevel; ++j) {
                while (true) {
                    ...
                    if (!used.contains(secondaryIdx)) {
                        secondaries.add((PhysicalNode) registeredServer
                                .toArray()[secondaryIdx]);
    ...
    public Vector<PhysicalNode> getAllSortedNodes(DhtPath path,
    ...
    class VirtualNode implements Serializable {
        ...
        public VirtualNode(PhysicalNode primary,
                Vector<PhysicalNode> secondaries) {
            isLeaf = true;
            this.vnodes = null;
            this.function = null;
